...
from agentsoul_web.app import routes as web_routes
...
mcp = FastMCP("AgentSoul", instructions=INSTRUCTIONS, stateless_http=True, json_response=True)
...
@mcp.tool()
def agentsoul_recall(query: str, anchors: dict[str, Any] | None = None,
                     limit: int = 5, minimum_score: float = 1.0) -> dict[str, Any]:
    """Recall relevant verified knowledge plus matching notes and entities."""
    results = retrieve(_knowledge(), query=query, anchors=anchors or {},
                       limit=max(1, min(limit, 20)), minimum_score=minimum_score)
    hybrid = _hybrid().search(query, limit=max(1, min(limit, 20)))
    return {"query": query, "knowledge_count": len(results),
            "context": render_context(results, max_chars=6000),
            "knowledge": [result.to_dict() for result in results],
            "notes": hybrid["notes"], "entities": hybrid["entities"]}


@mcp.tool()
def agentsoul_remember(kind: str, title: str, summary: str,
                       anchors: dict[str, Any] | None = None,
                       evidence: list[str] | None = None,
                       confidence: int = 2) -> dict[str, Any]:
    ...
    item = _knowledge().capture(kind=kind, title=title, summary=summary,
                                anchors=anchors or {}, evidence=evidence or [],
                                confidence=confidence)  # type: ignore[arg-type]
    return item.to_dict()


@mcp.tool()
def agentsoul_note(title: str, body: str, source: str | None = None,
...
@mcp.tool()
def agentsoul_entity(entity_type: str, name: str,
...
@mcp.tool()
def agentsoul_link(subject_id: str, predicate: str, object_id: str,
                   metadata: dict[str, Any] | None = None) -> dict[str, Any]:
...
@mcp.tool()
def agentsoul_contradict(knowledge_id: str, stated_value: str,
                         evidence: str | None = None) -> dict[str, Any]:
    """Record contradictory evidence without silently overwriting prior memory."""
    return _knowledge().contradict(knowledge_id, stated_value=stated_value,
...
def agentsoul_list(kind: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
    ...
    return [item.to_dict() for item in _knowledge().list_items(kind)[:max(1, min(limit, 100))]]  # type: ignore[arg-type]


class BearerTokenMiddleware(BaseHTTPMiddleware):
    """Optional bearer-token guard; the UI shell is public but all data routes are protected."""

    async def dispatch(self, request: Request, call_next):
        configured = os.environ.get("AGENTSOUL_API_TOKEN", "").strip()
        public = request.url.path in {"/", "/health", "/ui"}
        if not configured or public:
...
async def health(_: Request) -> JSONResponse:
    ...
    return JSONResponse({"status": "ok", "service": "agentsoul-mcp",
                         ...
                         "hybrid_database": str(_hybrid().initialise()),
                         "ui": "/ui"})
...
def create_app() -> Starlette:
    """Create an ASGI app exposing MCP, JSON API, health, and a lightweight web UI."""
...
def main() -> None:
    import uvicorn
    uvicorn.run("agentsoul_mcp.server:app",
                host=os.environ.get("AGENTSOUL_HOST", "127.0.0.1"),
                port=int(os.environ.get("AGENTSOUL_PORT", "8000")), reload=False)
