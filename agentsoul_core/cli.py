...
def build_parser() -> argparse.ArgumentParser:
    ...
    search.add_argument("--format", choices=("context", "json"), default="context")
    ...
    start.add_argument("--anchors", default="{}", help="JSON object")
    start.add_argument("--limit", type=int, default=5)
    start.add_argument("--max-chars", type=int, default=4000)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "migrate-claudsoul":
        result = migrate_claudsoul(args.source, store.initialise())
        print(json.dumps(result.__dict__, ensure_ascii=False))
        return 0
    if args.command == "emit":
        event = AgentEvent(event_type=args.event_type, provider=args.provider, project_id=args.project,
                           session_id=args.session, payload=_json_object(args.payload, "--payload"))
        store.append(event)
    ...
    if args.command == "knowledge-capture":
        item = knowledge.capture(kind=args.kind, title=args.title, summary=args.summary,
                                 confidence=args.confidence, anchors=_json_object(args.anchors, "--anchors"),
                                 evidence=args.evidence)
        print(json.dumps(item.to_dict(), ensure_ascii=False))
        return 0
    if args.command == "knowledge-confirm":
        print(json.dumps(knowledge.confirm(args.knowledge_id, args.evidence).to_dict(), ensure_ascii=False))
        return 0
    ...
    if args.command == "knowledge-search":
        results = retrieve(knowledge, query=args.query, anchors=_json_object(args.anchors, "--anchors"),
                           limit=args.limit, minimum_score=args.minimum_score)
        print(render_json(results) if args.format == "json" else render_context(results, max_chars=args.max_chars), end="")
        return 0
    ...
    if args.command == "session-start":
        ...
        if args.format == "json":
            print(json.dumps({"session": record.__dict__, "context": context}, ensure_ascii=False))
        else:
            print(f"<!-- agentsoul-session: {record.session_id} -->")
            print(context, end="")
        return 0
    success = None if args.success == "unknown" else args.success == "true"
    record = sessions.finish(args.session_id, outcome=args.outcome, success=success)
